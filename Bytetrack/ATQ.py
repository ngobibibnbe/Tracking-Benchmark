...
visits_with_frame=[]
min_detection_for_artificial_visits=0.0000 #minimum iou of the corresponding detected  bbox in the tracker for an artificial visit
epsilone=0.00000000000001#000001 # to avoid null distance in the inverse calculus 
min_observation_max_value =0.0#minimum probability of the highest probable bbox corresponding to an artificial visit
def iou (boxA,boxB):
...
def adding_atq(nbr_visit, output_file, feeder=False, 
               ...
               is_it_random = True, model=True,curated_artificial_visit=None, fairmot=False, for_re_id=False, min_observation_max_value=min_observation_max_value):
    ...
    if feeder==True:
                    ...
                    if max_frame> frame_id:
                        ...
                        if max_d >=0 :
                            ...
                            if max(observation)>=0.5:
                                ...
                                print("max observation is lower than threshold",max(observation))
                    else:
                        print("**not in the video",frame_id)
                
                if flag==True:
                    nbr_of_visits+=1
                    print("the visit :", visit, 'is rewarded', dbn_infos[str(frame_id)]["current"][np.argmax(observation)])#['track_id'])
        ...
        if curated_artificial_visit is None and feeder==True:
            ...
            for frame_id in labels.keys():
                pick_everything+=[(frame_id, individual_id) for individual_id in labels[frame_id].keys()]
            selection= random.sample(pick_everything, nbr_visit )
            
            selections_frame_id={}
            for frame_id, selected_ind_id in selection : 
                if frame_id not in selections_frame_id.keys():
                    selections_frame_id[frame_id]=[]
                selections_frame_id[frame_id].append(selected_ind_id)
                            ...
                            if max(observation)>0:
                                ...
                                """if is_it_random ==True and feeder==False: 
                                    ...
                                    if random.choice( [False, False, False, True] ) ==True:
                                        ...
                                        dbn_infos[str(frame_id)]["observation"][visitor_id]=normalized_values
                                        #np.random(loc=0, scale=1, size=(len(observation))).tolist() #***random of lengt between 0 and 1 
                                #dbn_infos[str(frame_id)]["observed"]=atq"""
