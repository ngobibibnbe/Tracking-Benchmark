...
import cv2
POWERFULNESS=50 #inverse of power of older observations
hungarian = True
confidence_threshold = 0.5#.067 for pigs  #### Our mod 
confidence_on_hmm_choice=11#1#1  #1 equivaut à une normalisation basique #1.5
...
def process_forwad_backward(track_with_observation,nbr_visit="",pigs_HMM=True, json_save_path="Bytetrack/videos/GR77_20200512_111314_with_atq_tracking_with_HMM_result.json", video_path="Bytetrack/videos/GR77_20200512_111314.mp4"):
    """_summary_
    ...
    min_frame = min([int(i) for i in list(data.keys())])
    ...
    print("********************the size of identities is", len(identities_list))
    ...
    for identity in identities:
        b[str(identity)]={}
    initial_distribution = np.array([1/len(data[str(min_frame+1)]["current"]) for i in data[str(min_frame+1)]["current"] ])
    for frame_id, value in data.items():
        
        if  frame_id!=str(min_frame) and int(frame_id)<max_frame: #int(frame)>100: #(frame!="0" and int(frame)%25==0) or
    ...
    for identity in identities_list :#[:15]:
        if True: # identity=='71':
            identities_list_2.append(identity)
            #print(identity,"ok")
            ...
            """with open("/home/sophie/uncertain-identity-aware-tracking/Bytetrack/L.json", 'w') as file:
                json.dump(L,file)"""
            #exit(0)
            
    identities_list=identities_list_2 #in case i make a selection on the identities that we will execute 
    ...
    def get_proba_of_track_id(L,data):
        ...
        for idx_t, t in enumerate( V[1:] ):
            ...
            for identity in identities_list:
                all_known_track = [i for i in range(1,16,1) ] ###dble check?
    ...
    for idx_t, t in enumerate( V[1:] ): 
        ...
        for idx, identity in enumerate(identities_list):
        ...
        if hungarian == True:
            ...
            try:
                ...
                print("******excetion on this matrix: ",t,  e)#,t,identities_list[3], L[identities_list[3]]["t="+str(t)], matrice)            
            for idx, row in enumerate(row_ind):
                data[str(t)]["current"][row]["atq"] = identities_list[col_ind[idx]]
                
            for idx, track in enumerate(data[str(t)]["current"]):
                data[str(t)]["current"][idx]["atq"] = None
            for idx, row in enumerate(row_ind):
                #print(t,row, col_ind[idx], L[identities_list[col_ind[idx]]]["t="+str(t)][idx])
                #print(identities_list[col_ind[idx]])
    ...
    '''track_file=track_with_observation.split("DBN_result")[0]+"tracking_result.json"  # Home_folder+"/videos/GR77_20200512_111314tracking_result.json"
    ...
    while ret_val and frame_id<V[-1]: 
        ...
        if str(frame_id) in data.keys() and str(frame_id)!=str(min_frame):
            if (frame_id!=str(min_frame)) :
                #print(frame_id)
                ...
                for idx,track in enumerate(data[str(frame_id)]["current"]):
                    ...
                    tlwh = track["location"]
                    #print(track)
                    #print(frame_id, track)
                    atq= track["atq"] 
                    tid= str(track_id)   #"None"#
                    if (atq is not None):
                        #tid=str(atq)
                    ...
                    cv2.putText(frame, str(tid_text),(int(tlwh[0]), int(tlwh[1])),0, 5e-3 * 100, (0,255,0),2)
...
dataset_name = "MOT17-04-FRCNN"#folders[0]

"""
base_path= "/home/sophie/uncertain-identity-aware-tracking/Bytetrack/YOLOX_outputs/yolox_x_ablation/videos/"+dataset_name+"/gt_video"
...
gt_video_path="/home/sophie/uncertain-identity-aware-tracking/Bytetrack/YOLOX_outputs/yolox_x_ablation/videos/"+dataset_name+"/gt_video.mp4"
